...
use crate::{auth::Mechanism, Address, ClientName, ConnectionKind, Domain, Error, ParseArgsError};
...
// TODO: merge this type with `Mailbox`
/// <https://www.rfc-editor.org/rfc/rfc3461>
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
            ...
            Some((key, value)) if key.eq_ignore_ascii_case(b"NOTIFY") => {
                ...
                for v in value.split(|c| *c == b',') {
                    ...
                    match (v, &mut notify) {
                        ...
                        (value, None) if value.eq_ignore_ascii_case(FAILURE) => {
                            notify = Some(NotifyOn::Some {
                                success: false,
                                failure: true,
                                delay: false,
                ...
                self.notify_on = notify.ok_or(ParseArgsError::InvalidArgs)?;
