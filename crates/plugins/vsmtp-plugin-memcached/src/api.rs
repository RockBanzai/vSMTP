...
/// This plugin exposes methods to open a pool of connections to a memcached server using
/// Rhai.
...
pub mod memcached {
    ...
    /// # Example
    ///
    /// Build a service in `services/cache.rhai`;
    ///
    /// ```text
    /// // Import the plugin stored in the `plugins` directory.
    /// import "plugins/libvsmtp_plugin_memcached" as cache;
    ...
    /// Gets multiple value from multiple key from the server.
    ///
    /// # Args
    ///
    ...
    /// Gets multiple value from multiple key from the server with their `cas_id`, expiration seconds, key and value.
    ///
    /// # Args
    ///
    /// * `keys` - The keys you want to get the values from
