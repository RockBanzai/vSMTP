...
impl std::fmt::Display for Signature {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let Self {
            ...
            query_method,
            auid: _,
            signature_timestamp: _,
            expire_time: _,
            body_length: _,
            headers_field,
            copy_header_fields: _,
            body_hash,
            signature,
            raw: _,
        } = self;
        ...
        let query_method = query_method
            ...
            .collect::<Vec<_>>()
            .join(":");
        let headers_field = headers_field
            .iter()
            .map(ToString::to_string)
            .collect::<Vec<_>>()
            .join(":");
        ...
        f.write_fmt(format_args!(
            "DKIM-Signature: v={version}; a={signing_algorithm}; d={sdid}; s={selector};\r\n\
            \tc={canonicalization}; q={query_method}; h={headers_field};\r\n\
            \tbh={body_hash};\r\n\tb={signature}",
        ))?;
